...
class D3QN:
    ...
    def learn(self, logger):
        ...
        with torch.no_grad():
            ...
            max_actions = torch.argmax(self.q_eval.forward(next_states_tensor), dim=-1)
        ...
        logger.store(Eval_Q=q.mean().item(), Target_Q=target.mean().item(),
    ...
    with wandb.init(project=config.proj_name, name=config.run_name, config=config):
        ...
        while steps < int(config.num_steps / num_procs()):
            ...
            if steps % config.save_every == 0:
                ...
                save(config, save_name="_", model=agent.q_eval, wandb=wandb, ep=config.seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_name", type=str, default="DQ3N", help="Run name, default: baseline")
