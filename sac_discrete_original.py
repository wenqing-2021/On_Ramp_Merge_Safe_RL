...
class ReplayBuffer:
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        if self.n_step != 1:
            self.n_step_buff.append(state, action, reward)
            if self.n_step_buff.is_full():
                state, action, reward = self.n_step_buff.get()
    ...
    def sample(self):
        ...
        return (states, actions, rewards, next_states, dones), tree_idx, ISweight
...
class SAC(nn.Module):
    ...
    def learn(self, experiences, gamma, ISweight, logger=None):
        ...
        logger.store(LossPi=actor_loss.item(), alpha_loss=alpha_loss.item(), critic1_loss=critic1_loss.item(),
                     critic2_loss=critic2_loss.item(), alpha=current_alpha.item(),
                     critic1=q1.mean().item(), critic2=q2.mean().item(), entropy=entropy.mean().item())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_step', type=int, default=1, help='1 or 3')
