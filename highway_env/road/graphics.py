...
class RoadGraphics(object):
    ...
    @staticmethod
    def display_traffic(road: Road, surface: WorldSurface, simulation_frequency: int = 15, offscreen: bool = False,config: Optional[dict] = None) \
            -> None:
        """
        ...
        """
        if road.record_history and not config['only_show_ego_history']:
            for v in road.vehicles:
                VehicleGraphics.display_history(v, surface, simulation=simulation_frequency, offscreen=offscreen, frequency=config['show_history_frequency'], duration=config['show_history_duration'])
        elif road.record_history and config['only_show_ego_history']:
            VehicleGraphics.display_history(road.vehicles[-1], surface, simulation=simulation_frequency, offscreen=offscreen, duration=config['show_history_duration'], frequency=config['show_history_frequency'])
        for v in road.vehicles:
