...
class GameObservation(ObservationType):
    """Observe the kinematics of nearby vehicles."""
    ...
    def observe(self) -> np.ndarray:
        ...
        # recompute the distance and velocity 
        if self.clip:
            for i in range(1, self.vehicles_count):
                if obs[i][0] == 1.0 :
                    if abs(obs[i][1]) > self.features_range['x'][1]:
                        obs[i][:] = 0
                else:
                    break
        ...
        # normalize 
        if self.normalize:
            x_position_range = self.features_range['x']
            x_velocity_range = self.features_range['vx']
            y_position_range = self.features_range['y']
            y_velocity_range = self.features_range['vy']

            obs[:,1] = utils.lmap(obs[:,1], [x_position_range[0], x_position_range[1]], [-1, 1])
            obs[:,2] = utils.lmap(obs[:,2], [y_position_range[0], y_position_range[1]], [-1, 1])
            obs[:,3] = utils.lmap(obs[:,3], [x_velocity_range[0], x_velocity_range[1]], [-1, 1])
            obs[:,4] = utils.lmap(obs[:,4], [y_velocity_range[0], y_velocity_range[1]], [-1, 1])
        
        
        # flatten
        # obs_flat = obs[1:].flatten()
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    ...
    elif config["type"] == "GameObservation":
        return GameObservation(env, **config)
    else:
